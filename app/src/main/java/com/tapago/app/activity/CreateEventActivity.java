...
import com.tapago.app.adapter.MultiSelectorAdapter;
import com.tapago.app.adapter.TicketCategoryAdapter;
import com.tapago.app.adapter.VoucherCategoryAdapter;
...
public class CreateEventActivity extends BaseActivity implements PaymentMethodNonceCreatedListener, BraintreeCancelListener, BraintreeErrorListener, DropInResult.DropInResultListener {
    ...
    private ArrayList<com.tapago.app.model.VoucherModel.Datum> vCategoryArrayList = new ArrayList<>();
    private ArrayList<com.tapago.app.model.TicketModel.Datum> ticketCategoryArrayList = new ArrayList<>();
    ...
    String startDate = "", endDate = "", startTime = "", endTime = "", locality = "", categoryId = "", userChoosenTask, bannerPath = "", thumbPath = "", checkImage = "", eventMessage = "", eventFees = "";
    ...
    ArrayList<com.tapago.app.model.GetVendorModel.Datum> vendorListUser = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            btnSubmit.setText(MySharedPreferences.getMySharedPreferences().getCreate_Event());
            ...
            txtName.setText(MySharedPreferences.getMySharedPreferences().getCreate_Event());
        ...
        edtBudget.addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                if (edtBudget.getText().toString().length() > 0 && edtEventFee.getText().toString().length() > 0) {
                    float sum = Float.valueOf(edtBudget.getText().toString()) + Float.valueOf(edtEventFee.getText().toString());
                    String firstNumberAsString = String.format("%.0f", sum);
                    edtEventAmount.setText(firstNumberAsString);
                } else {
                    edtEventAmount.setText("MT0.00");
                }
            }
...
//                Use only those fields which are required.
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.edtSelectVendor:
                try {
                    if (sb1.length() > 0) {
                        multiSelectorAdapter = new MultiSelectorAdapter(getActivity(), vendorListUser, sb1.toString());
                        ...
                        multiSelectorAdapter = new MultiSelectorAdapter(getActivity(), vendorListUser, "");
                        vendorSelection.rcList.setLayoutManager(new LinearLayoutManager(getActivity()));
                        vendorSelection.rcList.setAdapter(multiSelectorAdapter);
                    }
                    vendorSelection.txtOk.setOnClickListener(new View.OnClickListener() {
                        ...
                        public void onClick(View v) {
                            ...
                            for (int i = 0; i < vendorListUser.size(); i++) {
                                if (vendorListUser.get(i).isCheckBoxChecked()) {
                                    sb.append(vendorListUser.get(i).getVendorId()).append(",");
                                    sb1.append(vendorListUser.get(i).getVendorId()).append(",");
                                    listCheck.append(vendorListUser.get(i).getFirstName() + " " + vendorListUser.get(i).getLastName()).append(",");
                                }
                            }
                            if (sb.length() > 0) {
            ...
            call.enqueue(new Callback<CreateEventModel>() {
                ...
                public void onResponse(@NonNull Call<CreateEventModel> call, @NonNull Response<CreateEventModel> response) {
                    ...
                    if (response.isSuccessful()) {
                        basicModel = response.body();
                        if (Objects.requireNonNull(basicModel).getCode() == 200) {
                            Toast.makeText(CreateEventActivity.this, basicModel.getMessage(), Toast.LENGTH_SHORT).show();
    ...
    /**
     * Voucher Category Api
     */
    private void voucherCategoryApi() {
        if (AppUtils.isConnectedToInternet(getActivity())) {
            // showProgressDialog(getActivity());
            ...
            Call<VoucherModel> call;
            call = RetrofitRestClient.getInstance().voucherCategory(params);

            if (call == null) return;

            call.enqueue(new Callback<VoucherModel>() {
                @Override
                public void onResponse(@NonNull Call<VoucherModel> call, @NonNull Response<VoucherModel> response) {
                    hideProgressDialog();
                    final VoucherModel voucherModel;
                    if (response.isSuccessful()) {
                        voucherModel = response.body();
                        if (Objects.requireNonNull(voucherModel).getCode() == 200) {
                            rcCategory.setLayoutManager(new LinearLayoutManager(getActivity()));
                            vCategoryArrayList.addAll(voucherModel.getData());
                            voucherCategoryAdapter = new VoucherCategoryAdapter(getActivity(), voucherModel.getData());
                            rcCategory.setAdapter(voucherCategoryAdapter);
                        } else if (Objects.requireNonNull(voucherModel).getCode() == 999) {
                            logout();
                        } else {
                            showSnackBar(getActivity(), voucherModel.getMessage());
                        }
                    } else {
                        showSnackBar(getActivity(), response.message());
                    }
                ...
                @Override
                public void onFailure(@NonNull Call<VoucherModel> call, @NonNull Throwable t) {
                    hideProgressDialog();
                    if (t instanceof SocketTimeoutException) {
                        showSnackBar(getActivity(), getString(R.string.connection_timeout));
                    } else {
     ...
     * ticket Category Api
     */
    private void ticketCategoryApi() {
        if (AppUtils.isConnectedToInternet(getActivity())) {
            // showProgressDialog(getActivity());
            HashMap<String, String> params = new HashMap<>();
            params.put("user_id", MySharedPreferences.getMySharedPreferences().getUserId());
            ...
            Call<TicketModel> call;
            call = RetrofitRestClient.getInstance().ticketApi(params);

            if (call == null) return;

            call.enqueue(new Callback<TicketModel>() {
                @Override
                public void onResponse(@NonNull Call<TicketModel> call, @NonNull Response<TicketModel> response) {
                    hideProgressDialog();
                    final TicketModel ticketModel;
                    if (response.isSuccessful()) {
                        ticketModel = response.body();
                        if (Objects.requireNonNull(ticketModel).getCode() == 200) {
                            try {
                                rcTicket.setLayoutManager(new LinearLayoutManager(getActivity()));
                                ticketCategoryArrayList.addAll(ticketModel.getData());
                                ticketCategoryAdapter = new TicketCategoryAdapter(getActivity(), ticketModel.getData());
                                rcTicket.setAdapter(ticketCategoryAdapter);
                                edtEventFee.setText(ticketModel.getEventFee());
                                eventFees = ticketModel.getEventFee();
                                eventMessage = ticketModel.getEventFeeMessage();
                            } catch (Exception e) {
                                e.printStackTrace();
                            }
                        } else if (Objects.requireNonNull(ticketModel).getCode() == 999) {
                            logout();
                        } else {
                            showSnackBar(getActivity(), ticketModel.getMessage());
                        }
                    } else {
                        showSnackBar(getActivity(), response.message());
                    }
                ...
                @Override
                public void onFailure(@NonNull Call<TicketModel> call, @NonNull Throwable t) {
                    hideProgressDialog();
                    if (t instanceof SocketTimeoutException) {
                        showSnackBar(getActivity(), getString(R.string.connection_timeout));
                    } else {
    ...
    private void categoryListApi() {
        if (AppUtils.isConnectedToInternet(getActivity())) {
            ...
            params.put("lang", MySharedPreferences.getMySharedPreferences().getLanguage());
            params.put("pagination_flag", "false");
     ...
     * venodor List
     */
    private void vendorListUserApi() {
        if (AppUtils.isConnectedToInternet(getActivity())) {
            HashMap<String, String> params = new HashMap<>();
            params.put("user_id", MySharedPreferences.getMySharedPreferences().getUserId());
            params.put("access_token", MySharedPreferences.getMySharedPreferences().getAccessToken());
